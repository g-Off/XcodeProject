...
public struct GlobalID: RawRepresentable {
	private static let hexCharacters = CharacterSet(charactersIn: "0123456789ABCDEF")
	
	public let rawValue: String
	
	public init?(rawValue: String) {
		if rawValue.utf16.flatMap({ UnicodeScalar($0) }).filter({ GlobalID.hexCharacters.contains($0) }).count == 24 {
	...
	public init() {
		self.rawValue = GlobalID.generator.next()
	}
	
	static func ids(from strings: [String]?) -> [GlobalID]? {
		return strings?.flatMap({ return GlobalID(rawValue: $0) })
	}
...
extension PropertyList {
	var globalID: GlobalID? {
		return GlobalID(rawValue: self.string)
	}
}
